...
//This function returns the row of legend
int BoardDataBase::FindlineOfHero(ifstream& myfile, int RowsInFile)
{
	int rowofhero;
	size_t found;
	string line;
...
void BoardDataBase::AllocateNewMapAndSetIt(ifstream& myfile, Board& b)
{
	...
	for (int i = 0; i < b.GetmapRows(); ++i) {
		...
		NewMap[i] = new char[strlen(s)];
		strcpy(NewMap[i], s);
	}
	b.SetMap(NewMap);
}

//This function Frees Databas Arr;
void BoardDataBase::FreeMapsArr() {
	for (int j = 0; j < DBsize; ++j) {
		for (int i = 0; i < Database[i].map.GetmapRows(); ++i)
			delete[]Database[0].map.GetRowPointer(i);
	}
	delete[] Database;
}
